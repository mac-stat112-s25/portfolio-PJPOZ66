...
A) Distribution, proportion, comparison, and additional insights
B) Either a histogram, boxplot, density plot or scatterplot
...
there are 10 easy hike, 22 moderate hikes, and 14 difficult hikes

2. Are the hikes evenly distributed among these categories, or are some more common than others?

No, the hikes are not evenly distributed. The Moderate category is the most common (22 hikes), while Easy and Difficult hikes are fewer, with 10 and 14. This suggests that moderate hikes are more prevalent in this dataset.
...
```{r}
# We add geom bar which displays bars representing the number of hikes in each category
ggplot(hikes, aes(x = rating)) +
  geom_bar()
```

```{r}
# We add the function labs which adds axis labels to our chart 
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```{r}
# In the code we put fill inside of geom bar and put the color blue which filled the bars a blue color on the chart
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
  labs(x = "Rating", y = "Number of hikes")
```
...
```{r}
# We added color inside of geom bar and set it to orange which made the border color orange in our visualization
ggplot(hikes, aes(x = rating)) +
...
```{r}
# In our code we added theme minimal which in our plot made the bakground grid and axis lines more simplified
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  labs(x = "Rating", y = "Number of hikes") +
  theme_minimal()
...
The purpose of the + is to add layers to the plot. We use it to combine different elements of the plot

-   We added the bars using `geom_bar()`? Why "geom"?

Geom stands for geometric object and you need at least one geom in your code to display data

-   What does `labs()` stand for?

Labs stands for labels such as axis labels and the title

-   What's the difference between `color` and `fill`?

Color changes the color of the border of the bars while fill, fills in the interior of the bar plot with a color.

#### Part b {.unnumbered}

In general, bar charts allow us to examine the following properties of a *categorical* variable:

-   **observed categories**: What categories did we observe? The difficulty ratings of easy, moderate, and difficult

-   **variability between categories**: Are observations evenly spread out among the categories, or are some categories more common than others? The distribution of hikes is not even across diffuclty levels with there being way more moderate hikes.

We must then *translate* this information into the *context* of our analysis, here hikes in the Adirondacks. Summarize below what you learned from the bar chart, in context. I learned from the bar chart that there are many moderate hikes to do but if you are a begginner there is less options and even less options if you are a advanced

#### Part c {.unnumbered}

Is there anything you don't like about this barplot? For example: check out the x-axis again.

I don't like that the categories are alphabetical and not in order of difficulty level
